...
Docker 完整构建与运行态验收使用 `powershell -ExecutionPolicy Bypass -File scripts/verify_docker_release.ps1`；已有镜像的快速复核可附加 `-SkipBuild`。脚本检查 Compose 配置、三项服务健康、db-init 退出码、Worker 零重启、容器 Python 依赖一致性、`/app/.env` 不存在，以及 live/ready/前端 HTTP 响应。
